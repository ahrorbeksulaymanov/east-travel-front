...
const MiceCard = ({item, isHover = true} : {item: IMice, isHover?:boolean}) => {
    ...
    return(
        <div data-aos="fade-up" className="lg:col-span-4 sm:col-span-6 col-span-12">
            <div className={`bg-white rounded-[10px] relative transition hover:shadow-lg h-full ${isHover ? "hover:translate-y-[-5px]" : ""}`}>
                <Image className="w-[100%] rounded-[10px] object-cover md:h-[450px] sm:h-[300px]" src={FILE_URL + item?.photo} alt={item?.title} width={400} height={400} />
