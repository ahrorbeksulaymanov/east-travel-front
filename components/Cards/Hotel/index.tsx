...
const HotelCard = ({item, isHover = true} : {item: IHotel, isHover?: boolean}) => {
    ...
    return(
        <div data-aos="fade-up" className="lg:col-span-4 sm:col-span-6 col-span-12">
            <div className={`bg-white rounded-[10px] hotel-card transition h-full ${isHover ? "hover:translate-y-[-5px]" : ""}`}>
                <Image className="w-[100%] rounded-t-[10px] object-cover sm:h-[400px]" src={FILE_URL + item?.mainPhoto} alt={item?.title} width={400} height={400} />
